...
\begin{center}
  ...
  \foreach \i/\y/\c in {0/2.18/mDarkTeal,1/1.56/tealBorder,2/0.94/orange,3/0.32/violet}{
...
\begin{tikzpicture}[x=1cm,y=1cm,
  ...
  thinarr/.style={-{Latex[length=1.5mm]}, thin, mDarkTeal!60}]

  \path (-6.55,-1.12) rectangle (6.55,3.35);

  \node[font=\normalsize\bfseries,text=mDarkTeal] at (-4.63,2.85) {1. Local gradients};
...
\only<2->{
    ...
    \draw[thinarr]
    (-3.08,2.24) -- ([yshift=0.25cm]sync.west);

    \draw[thinarr]
    (-3.08,1.54) -- ([yshift=0.08cm]sync.west);

    \draw[thinarr]
    (-3.08,0.84) -- ([yshift=-0.08cm]sync.west);

    \draw[thinarr]
    (-3.08,0.14) -- ([yshift=-0.25cm]sync.west);
}
