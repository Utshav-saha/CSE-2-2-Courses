...
% Set metropolis theme with counter numbering
\usetheme[numbering=counter,progressbar=none]{metropolis}

% Match the exact font from the screenshots (Fira Sans)
...
% Required for the exact bordered info boxes shown in the screenshots
\usepackage[many]{tcolorbox} 

\setbeameroption{hide notes}

% ---------- Exact Color Palette Extracted from Images ----------
\definecolor{mDarkTeal}{HTML}{23373B}  % Header bg, Title text, Blue box border
...
\definecolor{mText}{HTML}{23373B}      % Default text color
\definecolor{mRed}{HTML}{B0443A}       % Accent red
...
% Force round bullets to match the screenshot
\setbeamertemplate{itemize item}{\textbullet}
\setbeamertemplate{itemize subitem}{$\circ$}

% Font weight adjustments to match the sleek look of the screenshots
...
% ---------- Reusable TikZ styles (adapted to new colors) ----------
\tikzset{
...
\transfade[duration=0.2]
% --- geometry grid ---------------------------------------------------------
% half-width 6.80 matches the takeaway box below (13.4cm text + 2x3.5pt inner sep)
% 4 cards of 2.80 + 3 gaps of 0.80 = 13.60  ->  centres at +-5.40, +-1.80
\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm,
  ...
  % constant bounding box: keeps the figure from shifting as overlays reveal rows
  \path (-6.80,-1.26) rectangle (6.80,3.76);

  % ---------- (a) caption ----------
  \node[capt] at (0,3.42)
    {\textbf{(a) Before.}~Each rank persists one disjoint shard $W_i$ of this unit's parameter tensor.};

  % ---------- top row: sharded state  (card 1.95 .. 3.00) ----------
  \foreach \i/\x/\c/\s in {0/-5.40/mDarkTeal/$W_0$,1/-1.80/tealBorder/$W_1$,2/1.80/orange/$W_2$,3/5.40/violet/$W_3$}{
    \draw[rank] (\x-1.40,1.95) rectangle (\x+1.40,3.00);
    \node[ranklab] at (\x,2.84) {GPU \i};
    \node[shard=\c,minimum width=1.00cm,minimum height=0.44cm] at (\x,2.46) {\s};
    \node[note] at (\x,2.08) {persistent: $|W|/N$};
  }

  % ---------- collective layer  (band 0.95 .. 1.55) ----------
  \only<2->{
    % one shard leaves each rank: arrow tinted by the owning rank
    \foreach \x/\c in {-5.40/mDarkTeal,-1.80/tealBorder,1.80/orange,5.40/violet}{
      \draw[flow,draw=\c!75!black] (\x,1.90) -- (\x,1.60);
    }
    \draw[rounded corners=5pt, draw=orange!75!black, fill=orange!9, thick]
      (-6.80,0.95) rectangle (6.80,1.55);
    \node[font=\normalsize\bfseries, text=orange!40!black] at (0,1.25) {ALL-GATHER};
    \node[bandtag,anchor=west] at (-6.60,1.25) {process group: $N=4$};
    \node[bandtag,anchor=east] at (6.60,1.25) {$\approx\tfrac{N-1}{N}|W|$ received per rank};
  }

  % ---------- bottom row: materialized parameter  (card -0.50 .. 0.55) ----------
  \only<3->{
    % every rank receives the same full tensor: arrows share one colour
    \foreach \x in {-5.40,-1.80,1.80,5.40}{\draw[flow,draw=orange!70!black] (\x,0.90) -- (\x,0.60);}
    \foreach \i/\x in {0/-5.40,1/-1.80,2/1.80,3/5.40}{
      \draw[rank] (\x-1.40,-0.50) rectangle (\x+1.40,0.55);
      \node[ranklab] at (\x,0.39) {GPU \i};
      % full tensor: 4 tiles of 0.50 + 3 gaps of 0.03 = 2.09 wide (vs. 1.00 shard above)
      \foreach \j/\dx/\c in {0/-0.795/mDarkTeal,1/-0.265/tealBorder,2/0.265/orange,3/0.795/violet}{
        \ifnum\i=\j\relax
          \draw[rounded corners=2pt,draw=\c!80!black,fill=\c!22,thick]
            (\x+\dx-0.25,-0.21) rectangle (\x+\dx+0.25,0.23);
        \else
          \draw[rounded corners=2pt,draw=\c!50!black,fill=\c!6,thin,densely dashed]
            (\x+\dx-0.25,-0.21) rectangle (\x+\dx+0.25,0.23);
        \fi
      }
      \node[note] at (\x,-0.37) {transient: $|W|$};
    }
    \node[capt] at (0,-0.92)
...
\end{center}
\vspace{0.03cm}
% \visible (not \only) reserves the box on every overlay, so the figure above
% does not shift upward when the takeaway appears.
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[font=\scriptsize,text=mDarkTeal!55,align=center] at (0,0.85) {Communication for the next unit can\\overlap with current computation.};
\end{tikzpicture}}
\par\vspace{0.18cm}
