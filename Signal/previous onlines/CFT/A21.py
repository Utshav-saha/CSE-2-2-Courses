...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        for u_idx, u_val in enumerate(self.u):
            ...
            cos_part = np.cos(np.pi * 2 * u_val * self.x)
            sin_part = np.sin(np.pi * 2 * u_val* self.x)

            for y_idx in range(len(self.y)):

                row = self.I[y_idx,:]
                a[y_idx, u_idx] = np.trapezoid(row*cos_part, self.x)
                b[y_idx, u_idx] = -np.trapezoid(row*sin_part, self.x)
        ...
        for v_idx, v_val in enumerate(self.v):
            ...
            cos_part = np.cos(np.pi * 2 * v_val * self.y)
            sin_part = np.sin(np.pi * 2 * v_val * self.y)
            ...
            for u_idx in range(u):
                ...
                real_val = a_col*cos_part + b_col*sin_part
                img_val = b_col*cos_part - a_col*sin_part

                real[v_idx, u_idx] = np.trapezoid(real_val,self.y)
                img[v_idx, u_idx] = np.trapezoid(img_val, self.y)
    ...
    def plot_magnitude(self):
        ...
        magnitude = 1+ np.hypot(real, imag)
...
class InverseCFT2D:
    ...
    def reconstruct(self):
        ...
        """
        # TODO: implement this method
        a = np.zeros((len(self.v), len(self.x)))
        b = np.zeros((len(self.v), len(self.x)))
        

        for x_idx, x_val in enumerate(self.x):

            cos_part = np.cos(np.pi * 2 * x_val * self.u)
            sin_part = np.sin(np.pi * 2 * x_val * self.u)

            for v_idx in range(len(self.v)):
                real = self.real[v_idx, :]
                img = self.imag[v_idx,:]

                a_val = real * cos_part - img * sin_part
                b_val = real * sin_part + img * cos_part

                a[v_idx, x_idx] = np.trapezoid(a_val, self.u)
                b[v_idx, x_idx] = np.trapezoid(b_val, self.u)


        image = np.zeros((len(self.y), len(self.x)))

        for y_idx,y_val in enumerate(self.y):

            cos_part = np.cos(np.pi*2 * y_val * self.v)
            sin_part = np.sin(np.pi*2 * y_val * self.v)

            for x_idx in range(len(self.x)):
                a_col = a[:, x_idx]
                b_col = b[:, x_idx]

                real_part = a_col*cos_part - b_col*sin_part
                image[y_idx,x_idx] = np.trapezoid(real_part, self.v)
...
def denoise_image(input_path, output_path=None, cutoff=15.0):
    ...
    denoised_image = np.clip(denoised_image, 0, None)
